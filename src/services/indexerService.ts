...
// Use Vercel API proxy in production to avoid mixed content (HTTPS->HTTP) issues
// In development, connect directly to the indexer
const INDEXER_API_URL = import.meta.env.PROD ? '' : (import.meta.env.VITE_INDEXER_API_URL || 'http://localhost:3001')
const API_PREFIX = import.meta.env.PROD ? '/api/indexer' : '/api'
